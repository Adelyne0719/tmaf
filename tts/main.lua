					...
					Wait.condition(function()
						...
						if locPassSecondary then
							...
							for _, tag in pairs(eventCard.getTags()) do --Oh my god.
								...
								elseif tag == 'eventSecondShuffleDeck' or tag == 'eventSecondShuffleDeckOnly' or tag == 'eventSecondShuffleEvent' then
									
									local eventDeckValid = false
									if eventDeck ~= nil then
										eventDeckValid = pcall(function() return eventDeck.getGUID() end)
									end
									
									local discardPiles = {}

									for _, obj in pairs(getAllObjects()) do
										if obj ~= eventCard and (not eventDeckValid or obj ~= eventDeck) then
											local locRotZ = obj.getRotation().z
											
											local isEventDiscard = false
											local gn = string.lower(obj.getGMNotes() or "")
											if gn == 'eventdiscard' or gn == 'event' then
												isEventDiscard = true
											elseif obj.type == 'Deck' then
												local cards = obj.getObjects()
												if cards and #cards > 0 then
													local cgn = string.lower(cards[1].gm_notes or "")
													if cgn == 'event' or cgn == 'eventdiscard' then
														isEventDiscard = true
													end
												end
											end

											if isEventDiscard and (locRotZ > 350 or locRotZ < 10) then
												table.insert(discardPiles, obj)
											end
										end
									end
									
									if not eventDeckValid and #discardPiles > 0 then
										eventDeck = discardPiles[1]
										table.remove(discardPiles, 1)
										eventDeckValid = true
									end
									
									if eventDeckValid then
										eventDeck.setPosition({20,1.6,3})
										eventDeck.setRotation({0,180,180})
										
										local objectsToMerge = {}
										for _, obj in ipairs(discardPiles) do
											table.insert(objectsToMerge, obj)
										end
										if tag == 'eventSecondShuffleEvent' or eventCard.getPosition().z > 2.5 then
											table.insert(objectsToMerge, eventCard)
										end
										
										local function mergeNext()
											if #objectsToMerge == 0 then
												Wait.time(function()
													for _, obj in pairs(getAllObjects()) do
														if obj.type == 'Deck' then
															local pos = obj.getPosition()
															if math.abs(pos.x - 20) < 2 and math.abs(pos.z - 3) < 2 then
																eventDeck = obj
																pcall(function() obj.shuffle() end)
																break
															end
														end
													end
											...
											if eventDeckValid and eventDeck and not eventDeck.isDestroyed() then
												targetDeck = eventDeck
											else
												for _, obj in pairs(getAllObjects()) do
											...
											local nextObj = table.remove(objectsToMerge, 1)
											if nextObj and not nextObj.isDestroyed() then
												if targetDeck then
													local newDeck = targetDeck.putObject(nextObj)
													if newDeck ~= nil then 
														eventDeck = newDeck 
													else
														eventDeckValid = false
													end
												else
													nextObj.setPosition({20,1.6,3})
													nextObj.setRotation({0,180,180})
													eventDeck = nextObj
													eventDeckValid = true
												end
											end
											
											Wait.time(mergeNext, 0.5)
										end
										
										Wait.time(mergeNext, 0.5)
									else
										eventCard.setPosition({20,1.6,3})
