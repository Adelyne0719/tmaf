...
function onLoad(saved_data)
	...
	setupEndMsg = 'Players, in any order, can now fill up their Tactical Belt with the available Tactical Gear tokens near the Item decks. Once it is done, they may start the game.'
...
function coopCustomToggle()
	...
	if coopMode then
		if coopCustom then
			coopCustom = false
			boarderTile.editButton({index = 9, label = 'CUSTOM COOP DISABLED', color = {0,0,0,0.8}})
		else
			coopCustom = true
			boarderTile.editButton({index = 9, label = 'CUSTOM COOP ENABLED', color = {0.075,0.38,0.75}})
		end
	end
end
				...
				if locCanShoot then
					...
					if enemyF.hasTag('healthCount') then
						...
						if R then
							...
							if not markedWeapon.hasTag('weapon') then
								locReroll = false
								mayRerollNextShoot = false
							end
					...
					if locGM == 'xyrianToken' then
						...
						if lifeforms == 'Primebloods' then
							if locGM == '' then
								...
								broadcastToAll('Blank token was drawn. Two Adult tokens will be added to the Intruder Bag.',lifeformColor)
						...
						elseif lifeforms == 'Neoflesh' then
							...
							if locGM == '' then
								...
								if locGM == 'ironcladToken' or locGM == 'firespitterToken' or locGM == 'slasherToken' or locGM == 'crawlmineToken' then
									...
									broadcastToAll('Adult token was drawn. Two Twitchling tokens will be added to the Intruder Bag.',lifeformColor)
								...
								elseif locGM == 'queenToken' then
									...
									if queenFBag.getQuantity() == 0 then
										if isQueenAlive() then
											...
											broadcastToAll('Queen token removed since the Queen is dead.', lifeformColor)
										end
									else
										if isQueenAlive() then
						...
						elseif lifeforms == 'Sangrevores' then
							...
							if locGM == '' then
								onObjectNumberTyped(adultBag, 'Red', 2)
								broadcastToAll('Blank token was drawn. Two Ghoul tokens will be added to the Intruder Bag.',lifeformColor)
						...
						elseif lifeforms == 'Carnomorph' then
							if locGM == '' then
								...
								broadcastToAll('Blank token was drawn. Three Metagorger tokens will be added to the Intruder Bag.',lifeformColor)
	...
	if scriptEnabled then
		...
		if container == intruderBag then
			...
			if object.getGMNotes() == 'xyrianToken' then
				broadcastToAll('Xyrian token drawn. Resolve a Xyrian Activation and replace Tracers with new Xyrians.', xyrianColor)
				if not sound4Used then
...
function intruderAttack(tile, intruder, attackedPlayerColor, tag)
	...
	if not (lifeforms == 'Neoflesh' and locIntruderName == 'breeder') then --Cultist never attacks.
		if locIntruderName == 'breeder' and lifeforms == 'Primebloods' then
			locIntruderName = 'drone'
		elseif lifeforms == 'Sangrevores' then
			if locIntruderName == 'adult' then
				locIntruderName = 'ghoul'
			elseif locIntruderName == 'breeder' then
				locIntruderName = 'specter'
			elseif locIntruderName == 'queen' then
				locIntruderName = 'king'
			end
		elseif lifeforms == 'Carnomorph' then
			if locIntruderName == 'creeper' then
				locIntruderName = 'metagorger'
			elseif locIntruderName == 'adult' then
				locIntruderName = 'shambler'
			elseif locIntruderName == 'breeder' then
				locIntruderName = 'fleshbeast'
			elseif locIntruderName == 'queen' then
				locIntruderName = 'butcher'
			end
		end
		
		if intruder == robot then
			locIntruderName = 'slasher robot'
		end
		...
		tile.createButton({
			...
			function_owner = Global,
			label          = 'ATK ' .. locButtonCount .. ' ' .. locIntruderName,
