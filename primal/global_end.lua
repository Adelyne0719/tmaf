        if t == targetTag then return true end
    end
    return false
end
...
function isObjectTerrainType(obj)
    ...
    if rule.event == "CardPlay" then
        if rule.count then
            ...
            if rule.tag and rule.tag ~= event_data.tag then return false end
            if rule.tags and not checkTagMatch(rule.tags, event_data.tag) then return false end
        end
    end
    if rule.event == "TurnStart" then
        if rule.zone and rule.zone ~= event_data.zone then return false end
        if rule.anyZone and rule.anyZone ~= event_data.anyZone then return false end
...
function checkTagMatch(ruleTags, targetTag)
    if not ruleTags or type(ruleTags) ~= "table" then return false end
