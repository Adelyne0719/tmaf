...
def _token_path():
    # hanspell_token.txt 경로 고정
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "hanspell_token.txt")
...
def update_token(agent):
    """
    update passportKey by scraping Naver spell checker page
    """
    html = agent.get(
        url=('https://search.naver.com/search.naver'
             '?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=맞춤법검사기')
    )
    m = re.search(r'passportKey=([a-zA-Z0-9]+)', html.text)
    if not m:
        # 토큰을 찾지 못하면 바로 예외 발생
        raise RuntimeError("Failed to extract passportKey from Naver page.")

    token = parse.unquote(m.group(1))
